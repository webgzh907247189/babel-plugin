...
module.exports = ({ types }, options) => {
	if (options.exclude) {
		const userExclude = Array.isArray(options.exclude) ? options.exclude : [options.exclude];
	...
	return {
		visitor: {
			JSXOpeningElement(astPath, state) {
				...
				const comColumn = astPath?.get?.("loc")?.get?.("start")?.get?.("column")?.node ?? "";
				...
				if (!onlyShowAwakeIdeMsg) {
					// 渲染的文件
					newPropRenderFileName = types.jSXAttribute(types.jSXIdentifier("data-render-file-name"), types.stringLiteral(fileName ?? ""));

					// 当前的 行
					newPropComponentLine = types.jSXAttribute(types.jSXIdentifier("data-line"), types.stringLiteral(String(comLine) ?? ""));
				}
				let showAwakeIdeMsg = "";

				if (isShowAwakeIdeMsg) {
					showAwakeIdeMsg = types.jSXAttribute(types.jSXIdentifier("data-awakeide"), types.stringLiteral(String(`${fileName}:${comLine}:${comColumn}`) ?? ""));
				}
				if (showCompleteFilePath) {
					const completeFilename = state.file?.opts?.filename;
					newPropComponentCompleteLine = types.jSXAttribute(types.jSXIdentifier("complete-filepath"), types.stringLiteral(String(`${completeFilename}:${comLine}:${comColumn}`) ?? ""));
				}
